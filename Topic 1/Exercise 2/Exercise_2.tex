...
\begin{Highlighting}[]
\NormalTok{final_function <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(pis, means, sigmas) \{}
    \CommentTok{# Mean Vector and covariance matrix}
\NormalTok{    meanvector <-}\StringTok{ }\KeywordTok{c}\NormalTok{()}
\NormalTok{    covmatrix <-}\StringTok{ }\DecValTok{0}

    \CommentTok{# Single loop for both correlation matrix and mean vector}
    \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(pis)) \{}
\NormalTok{        meanvector <-}\StringTok{ }\KeywordTok{c}\NormalTok{(meanvector, pis[i]}\OperatorTok{*}\NormalTok{means[,i])}
\NormalTok{        covmatrix <-}\StringTok{ }\NormalTok{covmatrix }\OperatorTok{+}\StringTok{ }\NormalTok{pis[i]}\OperatorTok{*}\NormalTok{(sigmas[[i]] }\OperatorTok{+}\StringTok{ }\NormalTok{means[,i]}\OperatorTok{%*%}\KeywordTok{t}\NormalTok{(means[,i]))}
\NormalTok{    \}}

    \CommentTok{# Calculating mean vector and covariance matrix}
\NormalTok{    meanvector <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rowSums}\NormalTok{(}\KeywordTok{matrix}\NormalTok{(meanvector, }\DataTypeTok{nrow=}\DecValTok{2}\NormalTok{)),}\DataTypeTok{nrow=}\DecValTok{2}\NormalTok{)}
\NormalTok{    covmatrix <-}\StringTok{ }\NormalTok{covmatrix }\OperatorTok{-}\StringTok{ }\NormalTok{meanvector}\OperatorTok{%*%}\KeywordTok{t}\NormalTok{(meanvector)}

    \CommentTok{# Correlation matrix}
\NormalTok{    delta <-}\StringTok{ }\KeywordTok{diag}\NormalTok{(}\KeywordTok{diag}\NormalTok{(covmatrix)}\OperatorTok{^}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\OperatorTok{/}\DecValTok{2}\NormalTok{))}
\NormalTok{    corrmatrix <-}\StringTok{ }\NormalTok{delta}\OperatorTok{%*%}\NormalTok{covmatrix}\OperatorTok{%*%}\NormalTok{delta}
